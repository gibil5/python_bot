...
class TeamsConversationBot(TeamsActivityHandler):
        ...
        for member in teams_members_added:
            if member.id != turn_context.activity.recipient.id:
                await turn_context.send_activity(
                    f"Welcome to the Tranzact Bot { member.given_name } { member.surname }. "
                )
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        if "helm" in text:
            await self._helm_command(turn_context)
            return
    ...
    async def _mention_activity(self, turn_context: TurnContext):
        ...
        reply_activity = MessageFactory.text(f"Hola {mention.text}")
    ...
    async def _send_card(self, turn_context: TurnContext, isUpdate):
        buttons = [
            ...
            # ),
            CardAction(
                type=ActionTypes.message_back, title="Execute Helm Command", text="helm"
            ),
        ]
        # if isUpdate:
        ...
        for member in team_members:
            ...
            async def send_message(tc2: TurnContext):
                return await tc2.send_activity(
                    f"Hola {member.name}. I'm a Tranzact's Teams Bot."
    ...
    async def _helm_command(self, turn_context: TurnContext):
        mention = Mention(
            mentioned=turn_context.activity.from_property,
            text=f"<at>{turn_context.activity.from_property.name}</at>",
            type="mention",
        )
        reply_activity = MessageFactory.text(f"Executing Helm Command: repo add {mention.text}")
        reply_activity.entities = [Mention().deserialize(mention.serialize())]
        helm_commands.RunCommand(REPO_ADD)
